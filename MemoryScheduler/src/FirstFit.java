...
public class FirstFit {
    ...
    public MemoryCell last; // Pointer for the last block allocated. (Used by GUI class)
    ...
    public void TimerScheduler() { // Timer that calls the method Schedule that removes the head of the arraylist of processes if avaliable and the try to allocate it.
    ...
    public synchronized void sortFreeBlocks() { // Sort the arraylist of freeblocks by M#ID
    ...
    public synchronized MemoryCell freeblocksList(String op, MemoryCell b, Process p) { //Synchronized Method to interact with the list of free blocks.
        if (op.compareTo("Remove") == 0) { //If string op equals to "Remove", removes the memory block from the List of free blocks.
            this.freeblocks.remove(b);
            this.sortFreeBlocks();
            return null;
        }
        if (op.compareTo("Add") == 0) { //If string op equals to "Add", adds the memory block in the List of free blocks.
            this.freeblocks.add(b);
            this.sortFreeBlocks();
            return null;
        }
        if (op.compareTo("Search") == 0) { //Searches the list of free blocks for the first block that has the size equal or greater than the process size.
    ...
    public synchronized void allocatedblocksList(String op, MemoryCell b, int index) { //Synchronized Method to interact with the list of allocated blocks.
        ...
        if (op.compareTo("Remove") == 0) { //If string op equals to "Remove", removes the memory block from the List allocated blocks.
            this.allocatedblocks.remove(b);
        }
        if (op.compareTo("Add") == 0) { //If string op equals to "Add", adds the memory block in the List of allocated blocks.
            this.allocatedblocks.add(b);
        }
        if (op.compareTo("Search") == 0) { //Searches for allocated blocks with dead processes.
            MemoryCell aux;
