...
public class NextFit {
    ...
    public MemoryCell nextfit; // Pointer for the last alocated block of NextFit algorithm.
    ...
    public ArrayList<MemoryCell> blocks = new ArrayList<>(); // Auxiliary arraylist of blocks to interact with GUI class.
    ...
    public void TimerScheduler() { // Timer that executes every 3 seconds calling the method Schedule to remove the header of the list of processes if  avaliable.
    ...
    public int getNextFit() { // Returns the las block allocated.
        return this.nextfit.mid;
    }

    public synchronized void sortBlocks() { // Sort by M#ID the arraylist of blocks. It is helpful for this arraylist that is used by the class GUI that makes the refresh of jpanel.
        Collections.sort(blocks, new Comparator<MemoryCell>() {
            @Override
    ...
    public synchronized void sortFreeBlocks() { //Sort by M#ID the arraylist of freeblocks.
    ...
    public void Schedule() { // Memory Scheduler
        ...
        if (processes.size() >= 1) {
            ...
            if (blocks.isEmpty()) {
                ...
                for (int i = nextfit.mid; i < blocks.size(); i++) { //Begins searching for a free block starting where it stopped last time.
                    aux = blocks.get(i);
                    m = freeblocksList("Search", aux, null);
                    if (m != null && m.process("Get", null) == null && p.getSize() <= m.getSize()) {
